...
def main():
    ...
    try:
        ...
        if againInput == "a":
            ...
            c.execute("DELETE FROM {0} WHERE Name = '{1}'".format(profession,name,)) #delete if they have done it before
            c.execute("INSERT INTO {0}(Name) VALUES (?)".format(profession),(name,)) #put name in Name column of appropriate database
            conn.commit()            
            stressTest()
        else:
            review()
    ...
    for line in Questions:
        ...
        while True:
            try: 
                answer = int(input())
                if 0 < answer < 6:
                    c.execute("UPDATE {0} SET {1} = {2} WHERE Name = '{3}'".format(profession,"Q"+str(QuestionNo), answer,name))
                    break
                else:
...
def results(TotalScore):
    ...
    if profession == "students":
        ...
        if TotalScore < 60:
            ...
            status = "high"
    c.execute("UPDATE {0} SET Status = '{1}' WHERE Name = '{2}'".format(profession,status,name))
    conn.commit()
...
def review():
    ...
    if reviewOption.lower() == "w":
        c.execute("SELECT * FROM '{0}' WHERE Name = '{1}'".format(profession,name,))
        testData = c.fetchone()
        for line in Questions:
            print(line)
            responseConvert(testData[i])
            i += 1
            time.sleep(1)
        print("Going back to main menu")
        main()
        ...
        while True:
            try:    
                ...
                while True:
                    if QuestionNo > 49 and profession == "students" or QuestionNo > 30 and profession == "teachers" or QuestionNo < 1:
                        print("There is no such question")
                        QuestionNo = int(input("Please type in the number of question you would like to review: "))
                    else:
                        break
                c.execute("SELECT {0} FROM '{1}' WHERE Name = '{2}'".format("Q"+str(QuestionNo),profession,name,))
                testData = c.fetchone()
                for line in Questions:
                    if i == QuestionNo:
                        print(line)#the response to the question is in the next line, sending the integer to response conver function
